...
class DataBase:
    ...
    def insert_user(self,guid, user, password):
        try:
            self.c.execute("INSERT INTO tb_users (Id, Login, Hash_Senha, Dt_Cookie, Permissao) VALUES (%s, %s, %s, %s, %s);",(guid, user, password, None,0))
            self.db.commit()
    ...
    def get_user(self, username):
        try:
			self.c.execute("SELECT Hash_Senha, Permissao, Id FROM tb_users WHERE Login = %s", [username])
