...
Using the same methodology, an attacker could now check what the application does when a game finishes and the result is sent to the web server. Analyzing the browser inspector once again reveals that a POST is made to the `/game` route, as can be seen in the next image: 
